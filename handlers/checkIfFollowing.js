...
const checkIfFollowing = async (email, navigation) => {
  ...
  if (token) {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/is_following/${email}`, {
        method: 'GET',
        headers: headers,
      });
      if (response.status === 200) {
        ...
        console.error('Error al chequear si se sigue a alguien:', response.status);
      }
    } catch (error) {
      const message =
