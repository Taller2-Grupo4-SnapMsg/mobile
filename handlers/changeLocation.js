...
const changeCountry = async (location, navigation) => {
  ...
  if (token) {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/users/location?new_location=${location}`, {
        method: 'PUT',
        headers: headers,
      });
      if (response.status === 200) {
        ...
        console.error('Error al actualizar location:', response.status);
      }
    } catch (error) {
      const message =
